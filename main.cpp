...
void saveCorrectedTrajectory(const std::vector<Pose>& originalPoses,
    ...
    // Create extrinsic transformation
    Transform X_opt(extrinsic[0], extrinsic[1], extrinsic[2]);
    
    // Apply extrinsic to each pose and save
    file << std::fixed << std::setprecision(6);
    for (const auto& pose : originalPoses) {
        Transform T_original = poseToTransform(pose);
        Transform T_corrected = X_opt * T_original;
        Pose corrected = transformToPose(T_corrected);
        
        file << pose.timestamp << " " 
             << corrected.x << " " 
             << corrected.y << " " 
             << corrected.theta << std::endl;
    }
...
int main(int argc, char** argv) {
    ...
    std::string wheel_file = "wheel_odom.txt";
    std::string lidar_file = "lidar_odom.txt";
    ...
    // Step 6: Save corrected trajectory
    std::cout << "\n[6/6] Saving corrected LiDAR trajectory..." << std::endl;
    saveCorrectedTrajectory(lidarPoses, extrinsic, "corrected_lidar_path.txt");
